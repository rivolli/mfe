...
simple and efficient learning algorithms like Naive Bayes (`naive.bayes`) and 
...
## Extract one landmarking measures with folds=2
mf.landmarking(Species ~ ., iris, features="naive.bayes", score="kappa")
```

There are some measures interested to evaluate the linear separability 
(`linear.discriminant`) and the attribute information (`decision.stumps`). 
