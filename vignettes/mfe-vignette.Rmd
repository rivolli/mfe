...
each group of measures. For instance, `mf.infotheo` and `mf.statistical` compute 
the information theoretical and the statistical measures, respectively. The following examples illustrate these cases:
...
dataset, statistical information, descriptors about information theoretical, measures designed to extract characteristics about the DT model based or landmarks which represent the performance of simple algorithms applied to the dataset. The following example show the available 
groups:
...
`post.processing` methods are applied only for the `propClass` meta-feature. For
...
The statistical group can use two additional parameter called `by.class` and `transform`. To the former the default is `by.class=FALSE` which means that the meta-features are computed without consider the classes values. Otherwise, the measure is extracted using the instances separated by class. In the latter, the default value is `transform=TRUE` which means that categorical attributes will
be transformed to numeric. Otherwise, they will be ignored. Note that if all attributes are categorical and the values is set to FALSE none result will be
obtained. The following example shows the use of these two definitions:
...
Note that, in the first example the values and the cardinality of the measure  are different since the correlation between the attributes were computed using the instances for each class separatelly.
...
and the common information shared between each attribute and the class in the dataset, respectively. To list the measures of this group use `ls.infotheo()`. The following examples illustrate these measures:
...
dataset, like its number of leaves (`leaves`) and the number of nodes (`nodes`) of the tree. The following examples illustrate these measures:
...
simple and efficient learning algorithms like Naive Bayes (`nb`) and 
1-Nearest Neighbor (`nn`). The following examples illustrate these
 measures:
...
The performance extraction of these measures without a cross validation step can
 cause model overfitting in the data. Therefore the `mf.landmarking` function 
...
parameters `bins` can be used to define the number of values to be returned. The
 ...
 is a way to obtain all values from the measure and has the same effect of the use of an empty list. The following code illustrate 
