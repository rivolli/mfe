...
#'
#' Subsampling-based Relative Landmarking measures are landmarking measures  
#' using subsampling strategy.
#'
...
#' @param size The percentage of examples subsampled. (Default: 0.9)
...
#' relative(Species ~ ., iris, size=0.9, score="balanced.accuracy")
#' @export
relative <- function(...) {
  UseMethod("relative")
}
...
relative.default <- function(x, y, features="all",
                                   summary=c("mean", "sd"), size=0.9, folds=10, 
                                   score="accuracy", ...) {
  if(!is.data.frame(x)) {
